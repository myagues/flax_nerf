...
def render_rays(rays_o, rays_d, near, far, config, rng):
    """Render rays for the coarse model.
    Args:
        rays: (4, *) generated rays
        config: model and rendering config
        rng: random key
    Returns:
        pts: (num_samples, 3) points in space to evaluate model at
        z_vals: (num_samples) depths of the sampled positions
    """
    ...
    if config.perturb and rng is not None:
        ...
        upper = jnp.concatenate([mids, z_vals[-1:]])
        lower = jnp.concatenate([z_vals[:1], mids])
        # stratified samples in those intervals
        t_rand = random.uniform(rng, shape=z_vals.shape)
        z_vals = lower + (upper - lower) * t_rand
    z_vals = z_vals.astype(rays_d.dtype)
    pts = rays_o + jnp.einsum("i,j", z_vals, rays_d)
    return pts, z_vals


def render_rays_fine(rays_o, rays_d, z_vals, weights, rng, num_importance, perturbation=True):
    """Render rays for the fine model.
    Args:
        rays: (2, 3) origin and direction generated rays
        z_vals: (num_samples) depths of the sampled positions
        weights: (num_samples) weights assigned to each sampled color for the coarse model
        num_importance: number of samples used in the fine model
        perturbation: whether to apply jitter on each ray or not
        rng: random key
    Returns:
        pts: (num_samples + num_importance, 3) points in space to evaluate model at
        z_vals: (num_samples + num_importance) depths of the sampled positions
        z_samples: standard deviation of distances along ray for each sample
    """
    z_vals_mid = 0.5 * (z_vals[1:] + z_vals[:-1])
    z_samples = sample_pdf(z_vals_mid, weights[1:-1], num_importance, perturbation, rng)
    ...
    z_vals = z_vals.astype(rays_d.dtype)
    pts = rays_o + jnp.einsum("i,j", z_vals, rays_d)
    return pts, z_vals, jnp.std(z_samples)
...
def sample_pdf(bins, weights, num_importance, perturbation, rng):
    ...
    Args:
        bins: (num_samples - 1) bins to sample from
        weights: (num_samples - 2) weights assigned to each sampled color for the coarse model
        num_importance: the number of samples to draw from the distribution
        perturbation: whether to apply jitter on each ray or not
        rng: random key
    Returns:
        samples: (num_importance) the sampled rays
    """
    # get pdf
    weights = jnp.clip(weights, 1e-5)  # prevent NaNs
    pdf = weights / jnp.sum(weights, keepdims=True)
    cdf = jnp.cumsum(pdf)
    cdf = jnp.pad(cdf, (1, 0), constant_values=0.0)

    # take uniform samples
    if perturbation:
        uni_samples = random.uniform(rng, shape=(num_importance,))
    else:
        uni_samples = jnp.linspace(0.0, 1.0, num_importance)

    # invert CDF
    idx = jnp.searchsorted(cdf, uni_samples, side="right")

    below = jnp.maximum(0, idx - 1)
    above = jnp.minimum(len(cdf) - 1, idx)
    inds_g = jnp.stack([below, above])

    cdf_g0, cdf_g1 = jnp.take_along_axis(cdf[None], inds_g, axis=1)
    bins_g0, bins_g1 = jnp.take_along_axis(bins[None], inds_g, axis=1)

    denom = cdf_g1 - cdf_g0
    denom = jnp.where(denom < 1e-5, 1.0, denom)
    t = (uni_samples - cdf_g0) / denom
    samples = bins_g0 + t * (bins_g1 - bins_g0)
    return samples


def raw2outputs(raw, z_vals, rays_d, rng, raw_noise_std, white_bkgd):
    """Transforms model's predictions to semantically meaningful values.
    Args:
        raw: (num_samples || num_importance, 4) prediction from model
        z_vals: (num_samples || num_importance) integration time
        rays_d: (3) direction of each ray
    ...
    Returns:
        ...
        depth_map: estimated distance to object
        disp_map: disparity map (inverse of depth map)
        rgb_map: (3) estimated RGB color of a ray
        weights: (num_samples || num_importance) weights assigned to each sampled color
    """

    # compute 'distance' (in time) between each integration time along the ray
    dists = z_vals[1:] - z_vals[:-1]

    # the 'distance' from the last integration time is infinity
    dists = jnp.pad(dists, (0, 1), constant_values=1e10)  # [num_samples]
    dists = dists.astype(z_vals.dtype)

    # multiply each distance by the norm of its corresponding direction ray
    # to convert to real world distance (accounts for non-unit directions)
    dists = dists * jnp.linalg.norm(rays_d)

    # extract RGB
    rgb = nn.sigmoid(raw[:, :3])  # [num_samples, 3]
    ...
    weights = jnp.pad(alpha_[:-1], (1, 0), constant_values=1.0)
    weights = alpha * jnp.cumprod(weights)  # [num_samples]

    # computed weighted color along the ray
    rgb_map = jnp.einsum("i,ij", weights, rgb)  # [3]

    # estimated depth map is expected distance
    depth_map = jnp.einsum("i,i", weights, z_vals)  # []

    # sum of weights along the ray (this value is in [0, 1] up to numerical error)
    acc_map = jnp.sum(weights)
